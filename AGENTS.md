...
- 一覧、絞り込み、入力、読込中・空・エラー、非同期応答など、ブラウザーで再現できる画面の検証は、Playwrightによるheadlessテストを既定にします。Edgeのデバッグ接続ができないことを、headlessで可能な検証のブロッカーにしません。自動テストでは既存のEdgeや利用者のログイン状態を使わず、テスト専用の一時ブラウザーで実行します。
- 本番と共通の画面・生成バインディング・Goサービスを利用します。実Azure接続のアプリ画面も、Wails標準serverと共通の本番Goサービスをheadlessで検証し、認証はGo側のAzure CLIを使用します。既存Edge/CDP設定やAzure Portalへのログインを前提にせず、Wailsの標準機能でブラウザー実行できる場合はそれを使い、テスト専用の画面や通信処理を別途実装しません。
- Computer Useは、Windowsアプリの起動・終了、WebView2への表示、ウィンドウ操作など、ネイティブ環境でしか確かめられない項目に限定します。ブラウザーで確認できるシナリオをComputer Useで繰り返し検証しません。
- 利用者への画面提示・使い勝手の確認は操作できるウィンドウで行います。headlessテスト、ネイティブ確認、利用者による動作合意、実Azure接続の検証は区別して記録します。Azure Portal自身へのログインが必要な設定値照合は別検証とし、Azure CLI認証と混同しません。未実施の検証は未検証と記録し、実行手順と結果の正本は [プロジェクト定義](docs/project.md#commands) とします。
