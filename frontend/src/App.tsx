...
import { AddModel as AddModelRequest, CheckOperation, PendingOperations } from "../bindings/github.com/nuitsjp/azfoundry-deck/internal/service/creationservice";
import { GetScenario, SetAddModelScenario, SetCreateScenario, SetScenario } from "../bindings/github.com/nuitsjp/azfoundry-deck/internal/mock/mockservice";
...
import AddModel, { type CreateOutcome, type CreateRequest, type ModelTarget, type PendingOperation } from "./AddModel";
...
export default function App() {
  ...
  // Additions an earlier run left unresolved are read when the dialog opens, so
  // a crash or a forced quit does not hide an operation that may still be
  // running in Azure. A read that fails leaves the notice empty rather than
  // claiming there is nothing outstanding; the send-time check still refuses a
  // duplicate.
  useEffect(() => {
    if (!addingModel) return;
    let active = true;
    void PendingOperations().then(operations => {
      if (!active) return;
      setPendingOperations(operations.map(operation => ({
        id: operation.id,
      ...
      if (active) setPendingOperations([]);
    });
    return () => { active = false; };
  }, [addingModel]);
  ...
  return (
    <main>
      <header className="app-header">
        ...
        <div className="header-actions">{showErrors ? <button className="back-button" onClick={() => setShowErrors(false)}>デプロイ一覧に戻る</button> : null}<div className="update-meta"><span>{result ? `最終取得 ${new Date(result.fetchedAt).toLocaleTimeString("ja-JP")}` : "まだ取得が完了していません"}</span></div><button className="primary" onClick={() => void refresh()} disabled={!ready || configuring}><span aria-hidden="true">↻</span> {loading ? "再取得" : "更新"}</button></div>
      </header>
      ...
      {addingModel ? <AddModel catalog={catalog} loadModels={loadAddModelCandidates} loadDeploymentNames={loadAddModelDeploymentNames} onCreate={addModel} onCheck={checkAddModel} pendingOperations={pendingOperations} onRefreshList={refresh} onCreateScenarioChange={scenario => { void SetCreateScenario(scenario); }} mock={isMock} onClose={() => setAddingModel(false)} /> : null}
