...
export default function AddModel({ catalog, loadModels, onClose }: Props) {
  ...
  const deployRule = "2〜64文字の半角英数字・アンダースコア・ハイフン・ピリオド。";
  const deployError = name && !/^[a-zA-Z0-9_.-]{2,64}$/.test(name) ? deployRule : "";
  ...
  return <dialog ref={dialog} className="add-dialog" aria-labelledby="add-title" onCancel={onClose}>
    <header className="add-heading"><div><h2 id="add-title" ref={heading} tabIndex={-1}>{titles[screen]}</h2></div>{screen === "done" ? null : <button className="back-button" onClick={onClose} aria-label="モデル追加を閉じる">閉じる</button>}</header>
    ...
    ].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p>{screen === "done" ? "追加完了の表示までを再現しました。一覧への保存は行っていません。" : "新規指定したリソースグループ、Foundry、モデルの順に作成する想定です。"}</p><div className="add-actions">{screen === "review" ? <><button className="back-button" onClick={() => setScreen("model")}>変更</button><button className="primary" onClick={() => setScreen("done")}>追加</button></> : <button className="primary" onClick={onClose}>閉じる</button>}</div></> : null}
  </dialog>;
}
